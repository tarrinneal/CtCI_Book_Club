...
test('does not modify a linked list with no dups', () => {
  expect(removeDups(noDupsLL)).toStrictEqual(noDupsLL);
  expect(removeDups(noDupsLL2)).toStrictEqual(noDupsLL2);
});

test('removes consecutive duplicates in a linked list', () => {
  expect(removeDups(consecDupsLL)).toStrictEqual(removeDupsSol(consecDupsLL));
  expect(removeDups(consecDupsLL2)).toStrictEqual(removeDupsSol(consecDupsLL2));
});

test('removes non consecutive duplicates from a linked list', () => {
  expect(removeDups(nonConsecDupsLL)).toStrictEqual(removeDupsSol(nonConsecDupsLL));
  expect(removeDups(nonConsecDupsLL2)).toStrictEqual(removeDupsSol(nonConsecDupsLL2));
})