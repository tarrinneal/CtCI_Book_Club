...
test('returns the appropriate node for k values in range', () => {
  ...
  expect(kthToLast(ll, 3)).toStrictEqual(kthToLastSol(ll, 3));
  // 2nd to last node would be 4
  expect(kthToLast(ll, 2)).toStrictEqual(kthToLastSol(ll, 2));
});

test('returns the last element for a k value of 1', () => {
  expect(kthToLast(ll, 1)).toStrictEqual(kthToLastSol(ll, 1));
});
